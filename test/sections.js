...
describe('sections api', function() {
  ...
  it('Get one', function (done) {
    testrail.getSection(1, function (err, section) {
  ...
  it('Get all - filtered', function (done) {
    testrail.getSections(1, { suite_id: 2 }, function (err, sections) {
      expect(err).to.be.null;
      expect(sections).to.be.an.array;
      done();
    });
  ...
  it('Add', function (done) {
    testrail.addSection(1, { "suite_id": 5, "name": "This is a new section", "parent_id": 10 }, function (err, section) {
      expect(err).to.be.null;
      expect(section).to.be.an.object;
      done();
    });
  ...
  it('Update', function (done) {
    testrail.updateSection(1, { "suite_id": 5, "name": "This is a new section", "parent_id": 10 }, function (err, section) {
      expect(err).to.be.null;
      expect(section).to.be.an.object;
      done();
    });
  ...
  it('Delete', function (done) {
    testrail.deleteSection(1, function (err, section) {
      expect(err).to.be.null;
      expect(section).to.be.an.object;
      done();
    });
