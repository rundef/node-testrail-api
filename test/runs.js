...
describe('runs api', function() {
  ...
  it('Get one', function (done) {
    testrail.getRun(1, function (err, run) {
  ...
  it('Get all', function (done) {
    testrail.getRuns(1, function (err, runs) {
      expect(err).to.be.null;
  ...
  it('Get all - filtered', function (done) {
    testrail.getRuns(1, { is_completed: 0, suite_id: 4 }, function (err, runs) {
      expect(err).to.be.null;
      expect(runs).to.be.an.array;
      done();
    });
  ...
  it('Add', function (done) {
    testrail.addRun(1, { "suite_id": 5, "name": "This is a new test run" }, function (err, run) {
      expect(err).to.be.null;
      expect(run).to.be.an.object;
      done();
    });
  ...
  it('Update', function (done) {
    testrail.updateRun(1, { "description": "A description for the test run" }, function (err, run) {
      expect(err).to.be.null;
      expect(run).to.be.an.object;
      done();
    });
  ...
  it('Close', function (done) {
    testrail.closeRun(1, function (err, run) {
      expect(err).to.be.null;
      expect(run).to.be.an.object;
      done();
    });
  ...
  it('Delete', function (done) {
    testrail.deleteRun(1, function (err, run) {
      expect(err).to.be.null;
      expect(run).to.be.an.object;
      done();
    });
