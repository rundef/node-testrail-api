...
describe('projects api', function() {
  ...
  it('Get one', function (done) {
    testrail.getProject(1, function (err, project) {
  ...
  it('Get all', function (done) {
    testrail.getProjects(function (err, projects) {
      expect(err).to.be.null;
  ...
  it('Get all - filtered', function (done) {
    testrail.getProjects({ is_completed: '0' }, function (err, projects) {
      expect(err).to.be.null;
      expect(projects).to.be.an.array;
      done();
    });
  ...
  it('Add', function (done) {
    testrail.addProject({ "is_completed": true }, function (err, project) {
      expect(err).to.be.null;
      expect(project).to.be.an.object;
      done();
    });
  ...
  it('Update', function (done) {
    testrail.updateProject(1, { "is_completed": true }, function (err, project) {
      expect(err).to.be.null;
      expect(project).to.be.an.object;
      done();
    });
  ...
  it('Delete', function (done) {
    testrail.deleteProject(1, function (err, project) {
      expect(err).to.be.null;
      expect(project).to.be.an.object;
      done();
    });
