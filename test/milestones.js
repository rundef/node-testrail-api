...
describe('milestones api', function() {
  ...
  it('Get one', function (done) {
    testrail.getMilestone(1, function (err, milestone) {
      expect(err).to.be.null;
      expect(milestone).to.be.an.object;
      done();
    });
  });
  ...
  it('Get all', function (done) {
    testrail.getMilestones(1, function (err, milestones) {
      expect(err).to.be.null;
  ...
  it('Get all - filtered', function (done) {
    testrail.getMilestones(1, { is_completed: 0 }, function (err, milestones) {
      expect(err).to.be.null;
      expect(milestones).to.be.an.array;
      done();
    });
  ...
  it('Add', function (done) {
    testrail.addMilestone(1, { "name": "Release 2.0", "due_on": 1394596385 }, function (err, milestone) {
      expect(err).to.be.null;
      expect(milestone).to.be.an.object;
      done();
    });
  ...
  it('Update', function (done) {
    testrail.updateMilestone(1, { "is_completed": 1 }, function (err, milestone) {
      expect(err).to.be.null;
      expect(milestone).to.be.an.object;
      done();
    });
  ...
  it('Delete', function (done) {
    testrail.deleteMilestone(1, function (err, milestone) {
      expect(err).to.be.null;
      expect(milestone).to.be.an.object;
      done();
    });
