...
describe('cases api', function() {
  ...
  it('Get one', function (done) {
    testrail.getCase(1, function (err, objCase) {
      expect(err).to.be.null;
      expect(objCase).to.be.an.object;
      done();
    });
  });
  ...
  it('Get all', function (done) {
    testrail.getCases(1, function (err, cases) {
      expect(err).to.be.null;
  ...
  it('Get all - filtered', function (done) {
    testrail.getCases(1, { suite_id: 2, section_id: 3, updated_by: '4,5,6'}, function (err, cases) {
      expect(err).to.be.null;
      expect(cases).to.be.an.array;
      done();
    });
  ...
  it('Add', function (done) {
    testrail.addCase(1, { "custom_preconds": "These are the preconditions for a test case" }, function (err, objCase) {
      expect(err).to.be.null;
      expect(objCase).to.be.an.object;
      done();
    });
  ...
  it('Update', function (done) {
    testrail.updateCase(1, { "custom_preconds": "These are the preconditions for a test case" }, function (err, objCase) {
      expect(err).to.be.null;
      expect(objCase).to.be.an.object;
      done();
    });
  ...
  it('Delete', function (done) {
    testrail.deleteCase(1, function (err, objCase) {
      expect(err).to.be.null;
      expect(objCase).to.be.an.object;
      done();
    });
